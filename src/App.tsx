...
export default function App({ session, onSession, onSave, connected, onReconnect }: AppProps) {
  ...
  useEffect(() => { document.title = (page === 'profiles' ? 'Профиль' : navigation.find(item => item.id === page)?.label || 'Главная') + ' · Since Us'; }, [page]);
  ...
  return <div className="app-shell">
    <aside className="sidebar glass">
      <button className="brand" onClick={() => go('home')} aria-label="Since Us — на главную"><span className="brand-symbol"><Heart fill="currentColor" strokeWidth={0} /></span><span>Since Us</span></button>
    ...
    <div className="main-area">
      ...
      <main className={`page-content page-${page}`} key={page}>
        ...
        {page === 'home' && <>
          {data.profiles.length === 1 && <section className="welcome-card"><HeartHandshake size={28} /><div><h2>Ваша история начинается здесь</h2><p>Фото профиля загрузится из Telegram. Укажите день, когда вы встретились, и пригласите любимого человека.</p></div><button className="secondary-button" onClick={() => go('profiles')}>Настроить профиль<ArrowRight size={15} /></button></section>}
